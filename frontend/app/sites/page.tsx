...
export default function SitesPage() {
  const {
    ...
    error,
    isLoading,
  } = useSWR<SiteMetrics[]>(`${API_BASE}/api/metrics/per-site`, fetcher);
    ...
    return (
      ...
      >
        <Alert severity="warning">
          No site metrics available. Run training first.
        </Alert>
      </Box>
    );
  }
  ...
  // Normalize and add row IDs
  const siteRows: SiteMetrics[] = Array.isArray(sites) ? sites : [];
  const rowsWithIds = siteRows.map((site, index) => ({
    id: index,
    ...site,
  }));
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
        Detailed performance metrics for each monitoring site in the Tri-County
        area.
      </Typography>
          ...
          <Box sx={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            ...
            <Box>
              <Typography variant="h6" color="success.main">
                {siteRows.filter((s) => s.pr_auc >= 0.7).length}
              </Typography>
              <Typography variant="body2" color="text.secondary">
            ...
            <Box>
              <Typography variant="h6" color="warning.main">
                {
                  siteRows.filter((s) => s.pr_auc >= 0.5 && s.pr_auc < 0.7)
                    .length
                }
              </Typography>
              <Typography variant="body2" color="text.secondary">
            ...
            <Box>
              <Typography variant="h6" color="error.main">
                {siteRows.filter((s) => s.pr_auc < 0.5).length}
              </Typography>
              <Typography variant="body2" color="text.secondary">
